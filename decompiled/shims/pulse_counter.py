...
# File: pulse_counter.pyc (Python 3.9)
...
class MCU_counter:
    ...
    def build_config(self):
        self._mcu.add_config_cmd('config_counter oid=%d pin=%s pull_up=%d' % (self._oid, self._pin, self._pullup))
        clock = self._mcu.get_query_slot(self._oid)
        self._poll_ticks = self._mcu.seconds_to_clock(self._poll_time)
        sample_ticks = self._mcu.seconds_to_clock(self._sample_time)
        self._mcu.add_config_cmd('query_counter oid=%d clock=%d poll_ticks=%d sample_ticks=%d' % (self._oid, clock, self._poll_ticks, sample_ticks), is_init=True)
