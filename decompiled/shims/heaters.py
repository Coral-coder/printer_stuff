...
#     decompiled/_disasm/heaters.txt
# =====================================================================

# Source Generated with Decompyle++
# File: heaters.pyc (Python 3.9)
...
class Heater:
    ...
    def __init__(self, config, sensor):
        ...
        self.min_extrude_temp = config.getfloat('min_extrude_temp', 1.7e+02, minval=self.min_temp, maxval=self.max_temp)
        is_fileoutput = self.printer.get_start_args().get('debugoutput') is not None
        if not self.min_extrude_temp <= 0.0:
            pass
        self.can_extrude = is_fileoutput
        ...
        pwm_cycle_time = config.getfloat('pwm_cycle_time', 0.1, above=0.0, maxval=self.pwm_delay)
    ...
    def set_pwm(self, read_time, value):
        if self.target_temp <= 0.0:
            value = 0.0
        if (read_time < self.next_pwm_time or self.last_pwm_value) and abs(value - self.last_pwm_value) < 0.05:
            return None
    ...
    def set_temp(self, degrees):
        ...
        if degrees:
            if degrees < self.min_temp or degrees > self.max_temp:
                raise self.printer.command_error('{"code":"key340", "msg":"Heaters %s Requested temperature (%.1f) out of range (%.1f:%.1f)", "values":["%s", %.1f, %.1f, %.1f]}' % (self.name, degrees, self.min_temp, self.max_temp, self.name, degrees, self.min_temp, self.max_temp))
        with self.lock:
    ...
    def check_busy(self, eventtime):
        with self.lock:
            pass
        self.control.check_busy(eventtime, self.smoothed_temp, self.target_temp)
    ...
    def stats(self, eventtime):
        ...
        if not target_temp:
            pass
        is_active = last_temp > 5e+01
        return (is_active, '%s: target=%.0f temp=%.1f pwm=%.3f' % (self.name, target_temp, last_temp, last_pwm_value))
        ...
        if target_temp >= 20 and target_temp <= 120:
            ...
            if self.long_temp:
                if self.old_temp <= 0.01 or self.old_temp < temp:
                    self.old_temp = temp
                    self.cnt_temp = 0
                    self.diff_tempa = 16.1 + 1.029 * (target_temp - 2e+01)
                    self.diff_tempb = 16.3 + 1.032 * (target_temp - 2e+01)
                elif self.old_temp > temp:
                    self.cnt_temp = self.cnt_temp + 1
                    if self.cnt_temp > 10:
                        self.long_temp = False
                    else:
                        self.diff_tempa = 19.1 + 1.006 * (target_temp - 2e+01)
                        self.diff_tempb = 19.3 + 1.009 * (target_temp - 2e+01)
            if self.heating and temp >= self.diff_tempb:
                self.heating = False
            elif self.heating and temp <= self.diff_tempa:
                self.heating = True
...
class ControlPID:
    ...
    def check_busy(self, eventtime, smoothed_temp, target_temp):
        ...
        if not abs(temp_diff) > self.max_delta:
            pass
        return abs(self.prev_temp_deriv) > PID_SETTLE_SLOPE
...
class PrinterHeaters:
    ...
    def __init__(self, config):
        ...
        gcode = self.printer.lookup_object('gcode')
        gcode.register_command('TURN_OFF_HEATERS', self.cmd_TURN_OFF_HEATERS, desc=self.cmd_TURN_OFF_HEATERS_help)
        gcode.register_command('M105', self.cmd_M105, when_not_ready=True)
        gcode.register_command('TEMPERATURE_WAIT', self.cmd_TEMPERATURE_WAIT, desc=self.cmd_TEMPERATURE_WAIT_help)
    ...
    def _wait_for_temperature(self, heater):
        ...
        if self.printer.is_shutdown() and heater.check_busy(eventtime):
            if self.can_break:
    ...
    def cmd_TEMPERATURE_WAIT(self, gcmd):
        ...
        reactor = self.printer.get_reactor()
        eventtime = reactor.monotonic()
        if not self.printer.is_shutdown() and self.can_break:
