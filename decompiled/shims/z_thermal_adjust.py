...
# File: z_thermal_adjust.pyc (Python 3.9)
...
class ZThermalAdjuster:
    ...
    def cmd_SET_Z_THERMAL_ADJUST(self, gcmd):
        ...
        if enable is not None and enable != self.adjust_enable:
        ...
        msg = 'enable: %s\ntemp_coeff: %f mm/degC\nref_temp: %.2f degC%s\n-------------------\nCurrent Z temp: %.2f degC\nApplied Z adjustment: %.4f mm' % (state, self.temp_coeff, self.ref_temperature, override, self.smoothed_temp, self.z_adjust_mm)
