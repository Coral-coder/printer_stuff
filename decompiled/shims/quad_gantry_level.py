...
# File: quad_gantry_level.pyc (Python 3.9)
...
class QuadGantryLevel:
    ...
    def __init__(self, config):
        ...
        if len(self.gantry_corners) < 2:
            raise config.error('{"code":"key214", "msg": "quad_gantry_level requires at least two gantry_corners" "values": []}')
        self.gcode = self.printer.lookup_object('gcode')
        self.gcode.register_command('QUAD_GANTRY_LEVEL', self.cmd_QUAD_GANTRY_LEVEL, desc=self.cmd_QUAD_GANTRY_LEVEL_help)
    ...
    def probe_finalize(self, offsets, positions):
        ...
        ppx0 = [
            ...
            z_positions[0]]
        ppx3 = [
            positions[3][0] + offsets[0],
            z_positions[3]]
        slope_x_pp03 = self.linefit(ppx0, ppx3)
        ppx1 = [
            positions[1][0] + offsets[0],
            z_positions[1]]
        ppx2 = [
            positions[2][0] + offsets[0],
            z_positions[2]]
        ...
        if adjust_max > self.max_adjust:
            raise self.gcode.error('{"code":"key215", "msg": "Aborting quad_gantry_level required adjustment %0.6f is greater than max_adjust %0.6f" "values": [%0.6f,%0.6f]}' % (adjust_max, self.max_adjust, adjust_max, self.max_adjust))
