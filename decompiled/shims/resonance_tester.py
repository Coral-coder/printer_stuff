    ...
    def run_test(self, axis, gcmd):
        ...
        try:
            if freq <= self.freq_end + 1e-06:
                ...
                max_v = accel * t_seg
                toolhead.cmd_M204(self.gcode.create_gcode_command('M204', 'M204', {
                    'S': accel }))
                L = 0.5 * accel * t_seg ** 2
...
class ResonanceTester:
    ...
    def __init__(self, config):
        ...
        self.gcode = self.printer.lookup_object('gcode')
        self.gcode.register_command('MEASURE_AXES_NOISE', self.cmd_MEASURE_AXES_NOISE, desc=self.cmd_MEASURE_AXES_NOISE_help)
        self.gcode.register_command('TEST_RESONANCES', self.cmd_TEST_RESONANCES, desc=self.cmd_TEST_RESONANCES_help)
        self.gcode.register_command('SHAPER_CALIBRATE', self.cmd_SHAPER_CALIBRATE, desc=self.cmd_SHAPER_CALIBRATE_help)
    ...
    def cmd_SHAPER_CALIBRATE(self, gcmd):
        ...
        for axis in calibrate_axes:
            ...
            gcmd.respond_info('Calculating the best input shaper parameters for %s axis' % (axis_name,))
            ...
            if input_shaper is not None:
                helper.apply_params(input_shaper, axis_name, best_shaper.name, best_shaper.freq)
            helper.save_params(configfile, axis_name, best_shaper.name, best_shaper.freq)
            csv_name = self.save_calibration_data('calibration_data', name_suffix, helper, axis, calibration_data[axis], all_shapers)
            ...
            gcmd.respond_info('Shaper calibration data written to %s file' % (csv_name,))
    ...
    def cmd_MEASURE_AXES_NOISE(self, gcmd):
        ...
        for chip_axis, aclient in raw_values:
            if not aclient.has_valid_samples():
                raise gcmd.error('%s-axis accelerometer measured no data' % (chip_axis,))
            data = helper.process_accelerometer_data(aclient)
            vx = data.psd_x.mean()
            vy = data.psd_y.mean()
            vz = data.psd_z.mean()
            gcmd.respond_info('Axes noise for %s-axis accelerometer: %.6f (x), %.6f (y), %.6f (z)' % (chip_axis, vx, vy, vz))
