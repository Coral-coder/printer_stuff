...
# File: temperature_fan.pyc (Python 3.9)
...
class TemperatureFan:
    ...
    def cmd_SET_TEMPERATURE_FAN_TARGET(self, gcmd):
        ...
        if min_speed > max_speed:
            raise self.printer.command_error('Requested min speed (%.1f) is greater than max speed (%.1f)' % (min_speed, max_speed))
    ...
    def set_temp(self, degrees):
        if degrees:
            if degrees < self.min_temp or degrees > self.max_temp:
                raise self.printer.command_error('{"code":"key339", "msg":"TemperatureFan %s Requested temperature (%.1f) out of range (%.1f:%.1f)", "values":["%s", %.1f, %.1f, %.1f]}' % (self.name, degrees, self.min_temp, self.max_temp, self.name, degrees, self.min_temp, self.max_temp))
