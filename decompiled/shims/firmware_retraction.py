...
# File: firmware_retraction.pyc (Python 3.9)
...
class FirmwareRetraction:
    ...
    def cmd_GET_RETRACTION(self, gcmd):
        gcmd.respond_info('RETRACT_LENGTH=%.5f RETRACT_SPEED=%.5f UNRETRACT_EXTRA_LENGTH=%.5f UNRETRACT_SPEED=%.5f' % (self.retract_length, self.retract_speed, self.unretract_extra_length, self.unretract_speed))

    
    def cmd_G10(self, gcmd):
        if not self.is_retracted:
            self.gcode.run_script_from_command('SAVE_GCODE_STATE NAME=_retract_state\nG91\nG1 E-%.5f F%d\nRESTORE_GCODE_STATE NAME=_retract_state' % (self.retract_length, self.retract_speed * 60))
            self.is_retracted = True

    
    def cmd_G11(self, gcmd):
        if self.is_retracted:
            self.gcode.run_script_from_command('SAVE_GCODE_STATE NAME=_retract_state\nG91\nG1 E%.5f F%d\nRESTORE_GCODE_STATE NAME=_retract_state' % (self.unretract_length, self.unretract_speed * 60))
            self.is_retracted = False
