...
# File: smart_effector.pyc (Python 3.9)
...
class ControlPinHelper:
    ...
    def _build_config(self):
        ...
        self._mcu.add_config_cmd('config_digital_out oid=%d pin=%s value=%d default_value=%d max_duration=%d' % (self._oid, self._pin, self._start_value, self._start_value, 0))
...
class SmartEffectorEndstopWrapper:
    ...
    def __init__(self, config):
        ...
        if control_pin:
            ...
            self.gcode.register_command('RESET_SMART_EFFECTOR', self.cmd_RESET_SMART_EFFECTOR, desc=self.cmd_RESET_SMART_EFFECTOR_help)
        else:
            self.control_pin = None
        self.gcode.register_command('SET_SMART_EFFECTOR', self.cmd_SET_SMART_EFFECTOR, desc=self.cmd_SET_SMART_EFFECTOR_help)
