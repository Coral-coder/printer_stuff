...
class MCU_TMC_uart_bitbang:
    ...
    def build_config(self):
        ...
        self.mcu.add_config_cmd('config_tmcuart oid=%d rx_pin=%s pull_up=%d tx_pin=%s bit_time=%d' % (self.oid, self.rx_pin, self.pullup, self.tx_pin, bit_ticks))
        self.tmcuart_send_cmd = self.mcu.lookup_query_command('tmcuart_send oid=%c write=%*s read=%c', 'tmcuart_response oid=%c read=%*s', oid=self.oid, cq=self.cmd_queue, is_async=True)
    ...
    def reg_read(self, instance_id, addr, reg):
        ...
        params = self.tmcuart_send_cmd.send([
            self.oid,
            msg,
            10])
