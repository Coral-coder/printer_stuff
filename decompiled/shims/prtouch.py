...
class PRTouchEndstopWrapper:
    ...
    def _check_trigger(self, arg_index, fit_vals, unfit_vals, min_hold, max_hold):
        ...
        vals_p = [ x for x in (fit_vals_t) ]
        if not fit_vals_t[-2] > fit_vals_t[-2] or fit_vals_t[-2] > fit_vals_t[-3]:
            pass
        else:
            fit_vals_t[-1]
        return False
        max_val = max(fit_vals_t[0:self.cfg.pi_count - 3])
        if not fit_vals_t[-1] > max_val and fit_vals_t[-2] > max_val or fit_vals_t[-3] > max_val:
            return False
        ...
        for i in range(0, self.cfg.pi_count - 1):
            if (fit_vals_t[-1] - fit_vals_t[i]) / (self.cfg.pi_count - i) * 1.0 / self.cfg.pi_count < 0.8:
                return False
            if fit_vals[-1] < min_hold and fit_vals[-2] < min_hold / 2 or fit_vals[-3] < min_hold / 3:
                return False
    ...
    def _set_hot_temps(self, temp, fan_spd, wait, err = (False, 5)):
        ...
        if wait and self.ck_sys_sta() and abs(self.obj.heater_hot.target_temp - self.obj.heater_hot.smoothed_temp) > err and self.obj.heater_hot.target_temp > 0:
    ...
    def _probe_times(self, max_times, rdy_pos, speed_mm, min_dis_mm, max_z_err, min_hold, max_hold):
        ...
        self._move(now_pos[:2] + [
            rdy_pos[2]], self.cfg.g29_rdy_speed)
        self._move(rdy_pos, self.cfg.g29_xy_speed)
        for i in range(max_times):
            (o_index0, o_mm0, deal_sta) = self.probe_by_step(rdy_pos[:2] + [
                rdy_pos_z], speed_mm, min_dis_mm, min_hold, max_hold, True)
            if deal_sta and rdy_pos_z == rdy_pos[2]:
                rdy_pos_z += 2
                continue
            (o_index1, o_mm1, deal_sta) = self.probe_by_step(rdy_pos[:2] + [
                rdy_pos_z], speed_mm, min_dis_mm, min_hold, max_hold, True)
            o_mm = (o_mm0 + o_mm1) / 2
            if not math.fabs(o_mm0 - o_mm1) <= max_z_err or self.ck_sys_sta():
                pass
            else:
    ...
    def get_best_rdy_z(self, rdy_x, rdy_y, base_pos = (None,)):
        ...
        if p_mid[2] < self.cfg.bed_max_err:
            return p_mid[2]
        return self.cfg.bed_max_err
    ...
    def shake_motor(self, cnt):
        ...
        for i in range(int(cnt / 2)):
            ...
            self.obj.gcode.run_script_from_command('G1 X%.2f Y%.2f Z%.2f F600' % (now_pos[0] + self.cfg.shake_range, now_pos[1] + self.cfg.shake_range, now_pos[2] - self.cfg.shake_range / 2))
            self.obj.gcode.run_script_from_command('G1 X%.2f Y%.2f Z%.2f F600' % (now_pos[0] - self.cfg.shake_range, now_pos[1] + self.cfg.shake_range, now_pos[2] + self.cfg.shake_range / 2))
            if len(self.obj.toolhead.move_queue.queue) >= 4 and self.ck_sys_sta():
                self.obj.hx711s.delay_s(0.01)
                continue
                continue
    ...
    def clear_nozzle(self, hot_min_temp, hot_max_temp, bed_max_temp, min_hold, max_hold):
        ...
        max_x = self.cfg.clr_noz_start_x + self.cfg.clr_noz_len_x
        max_y = self.cfg.clr_noz_start_y + self.cfg.clr_noz_len_y
        ...
        src_pos = [
            min_x + random.uniform(0, self.cfg.clr_noz_len_x - self.cfg.pa_clr_dis_mm - 5),
            min_y + random.uniform(0, self.cfg.clr_noz_len_y),
            self.cfg.bed_max_err + 1,
            cur_pos[3]]
        end_pos = [
            src_pos[0] + self.cfg.pa_clr_dis_mm,
            src_pos[1],
            src_pos[2],
            src_pos[3]]
        self._set_hot_temps(temp=hot_min_temp, fan_spd=0, wait=True, err=10)
        src_pos[2] = self._probe_times(3, [
            src_pos[0] - 5,
            src_pos[1],
            src_pos[2]], self.cfg.g29_speed, 10, 0.2, min_hold * 2, max_hold)
        self._set_hot_temps(temp=hot_min_temp + 40, fan_spd=0, wait=False, err=10)
        end_pos[2] = self._probe_times(3, [
            end_pos[0] - 5,
            end_pos[1],
            end_pos[2]], self.cfg.g29_speed, 10, 0.2, min_hold * 2, max_hold)
        self._move(src_pos[:2] + [
            self.cfg.bed_max_err + 1], self.cfg.g29_xy_speed)
        self._move(src_pos[:2] + [
            src_pos[2] + 0.2], self.cfg.g29_rdy_speed)
        ...
        self._move(end_pos[:2] + [
            end_pos[2] + self.cfg.pa_clr_down_mm], self.cfg.g29_speed)
        self._set_hot_temps(temp=hot_min_temp, fan_spd=255, wait=True, err=5)
        self._move([
            end_pos[0] + 10,
            end_pos[1],
            end_pos[2] + 10], self.cfg.g29_speed)
        self._set_hot_temps(temp=hot_min_temp, fan_spd=0, wait=False)
        self._set_bed_temps(temp=bed_max_temp, wait=True, err=5)
        self._move(self.val.home_xy + [
            10], self.cfg.g29_xy_speed)
    ...
    def measure_gap(self, zero_z):
        ...
        p0_vals = None
        p1_vals = None
        rd_cnt = int(160 * (min_dis_mm / speed_mm))
        ...
        self._move(now_pos[:2] + [
            zero_z + min_dis_mm / 2,
            now_pos[3]], self.cfg.g29_rdy_speed)
        ...
        v_gap = 0.0
        for i in range(int(self.obj.hx711s.s_count)):
            if p0_diss[i] < p0_diss[i]:
                if p0_diss[i] < min_dis_mm:
                    pass
                else:
                    0
            elif p1_diss[i] < p1_diss[i]:
                if p1_diss[i] < min_dis_mm:
                    pass
                else:
                    0
            elif not gaps[i] <= gaps[i] or gaps[i] < 0.2:
                pass
            else:
                0
        v_cnt += 1
    ...
    def check_bed_mesh(self, auto_g29 = (True,)):
        ...
        self.val.rdy_pos = [
            [
                min_x + random.uniform(2.0, 5.0),
                min_y + random.uniform(2.0, 5.0),
                self.cfg.bed_max_err + 1.0],
            [
                min_x + random.uniform(2.0, 5.0),
                max_y - random.uniform(2.0, 5.0),
                self.cfg.bed_max_err + 1.0],
            [
                max_x - random.uniform(2.0, 5.0),
                max_y - random.uniform(2.0, 5.0),
                self.cfg.bed_max_err + 1.0],
            [
                max_x - random.uniform(2.0, 5.0),
        ...
        self._move([
            ...
            self.val.home_xy[1],
            self.cfg.bed_max_err + 1.0,
            self.obj.toolhead.get_position()[3]], self.cfg.g29_xy_speed)
        for i in range(4):
        ...
        if err_cnt >= 2:
            if auto_g29:
                ...
                raise self.obj.printer.command_error('{"code":"key501", "msg":"check_bed_mesh: Due to the large change of the hot bed, it can not print normally. Errs = [%.2f, %.2f, %.2f, %.2f]"}' % (errs[0], errs[1], errs[2], errs[3]))
        else:
    ...
    def raise_z_bef_g28(self):
        if self.obj.kin.limits[2][0] <= self.obj.kin.limits[2][1] and self.obj.toolhead.get_position()[2] < 5:
            self.obj.toolhead.manual_move(self.obj.toolhead.get_position()[:2] + [
                5], 5)
            self.obj.toolhead.wait_moves()
    ...
    def _cal_min_z(self, start_z, hx711_vals):
        ...
        dirzctl_params[1]['tick'] = (0x100000000 if dirzctl_params[1]['tick'] < dirzctl_start_tick else 0 + dirzctl_params[1]['tick'] - dirzctl_start_tick) / self.obj.dirzctl.mcu_freq
        ...
        tick_p = (0x100000000 if hx711_params[self.val.out_index]['nt'] < hx711_start_tick else 0 + hx711_params[self.val.out_index]['nt'] - hx711_start_tick) / self.obj.hx711s.mcu_freq
    ...
    def run_G28_Z(self):
        ...
        now_pos_sta0 = [
            now_pos[0] + 1 if int(time.time() * 1000) % 2 == 0 else -1 * random.uniform(10, 20),
            now_pos[1] + 1 if int(time.time() * 100) % 2 == 0 else -1 * random.uniform(10, 20),
            now_pos[2],
            now_pos[3]]
        mesh = self.obj.bed_mesh.get_mesh()
        self.obj.bed_mesh.set_mesh(None)
        self.obj.toolhead.set_position(now_pos, homing_axes=[
            2])
        ...
        is_uped = False
        for i in range(10):
            self.obj.toolhead.set_position(now_pos_sta0[:2] + [
                2 if i == 0 else 0,
                now_pos_sta0[3]], homing_axes=[
                2])
            ...
            if not deal_sta and is_uped:
                is_uped = True
                self.obj.toolhead.set_position(now_pos_sta0[:2] + [
                    0,
                    now_pos_sta0[3]], homing_axes=[
                    2])
                self._move(now_pos_sta0[:2] + [
                    5,
                    now_pos_sta0[3]], self.cfg.g29_rdy_speed)
            if out_mm < out_mm or out_mm < 1:
                pass
            else:
                -1
        if not deal_sta:
            ...
            self.obj.toolhead.set_position(now_pos_sta0[:2] + [
                -0.5,
                now_pos[3]], homing_axes=[
                2])
            self._move(now_pos_sta0[:2] + [
                2,
                now_pos[3]], self.cfg.g29_rdy_speed)
            now_pos = [
                now_pos[0] + random.uniform(-1.0, 1.0),
                now_pos[1] + random.uniform(-1.0, 1.0),
                2,
                now_pos[3]]
            ...
            self.obj.toolhead.set_position(now_pos[:2] + [
                now_pos[2] - out_mms[2] - self.cfg.self_z_offset,
                now_pos[3]], homing_axes=[
                2])
            self.obj.bed_mesh.set_mesh(mesh)
            self._move(now_pos[:2] + [
    ...
    def run_G29_Z(self):
        ...
        if self.val.g29_cnt == x_cnt * y_cnt:
            ...
            self._move([
                now_pos[0],
                now_pos[1],
                self.cfg.bed_max_err + 1], self.cfg.g29_rdy_speed)
            self._move([
                self.val.home_xy[0],
                self.val.home_xy[1],
                self.cfg.bed_max_err + 1], self.cfg.g29_xy_speed)
            home_z = self._probe_times(3, [
                self.val.home_xy[0],
                self.val.home_xy[1],
                self.cfg.bed_max_err + 1], self.cfg.g29_speed, self.cfg.bed_max_err * 2, 0.2, self.cfg.min_hold, self.cfg.max_hold)
    ...
    def cmd_PRTOUCH_TEST(self, gcmd):
        ...
        self._move([
            rdy_x,
            rdy_y,
            rdy_z], self.cfg.g29_xy_speed)
        self.probe_by_step([
            rdy_x,
            rdy_y,
            rdy_z], speed, 50, min_hold, max_hold, True)
    ...
    def cmd_MEASURE_GAP_TEST(self, gcmd):
        ...
        n_index = [
            ...
            2]
        self._move([
            self.val.rdy_pos[n_index[gap_index]][0],
            self.val.rdy_pos[n_index[gap_index]][1],
            self.cfg.bed_max_err + 1.0], self.cfg.g29_xy_speed)
        zero_z = self._probe_times(10, [
            self.val.rdy_pos[n_index[gap_index]][0],
            self.val.rdy_pos[n_index[gap_index]][1],
            self.cfg.bed_max_err + 1.0], self.cfg.g29_speed, 10, 0.05, self.cfg.min_hold, self.cfg.max_hold)
