...
class GCodeMacro:
    ...
    def __init__(self, config):
        if len(config.get_name().split()) > 2:
            raise config.error('{"code":"key166", "msg": "Name of section \'%s\' contains illegal whitespace", "values": ["%s"]}' % (config.get_name(), config.get_name()))
        ...
        if self.rename_existing is not None:
            if self.gcode.is_traditional_gcode(self.alias) != self.gcode.is_traditional_gcode(self.rename_existing):
                raise config.error('{"code":"key167", "msg": "G-Code macro rename of different types (\'%s\' vs \'%s\')", "values": ["%s", "%s"]}' % (self.alias, self.rename_existing, self.alias, self.rename_existing))
            printer.register_event_handler('klippy:connect', self.handle_connect)
        else:
            self.gcode.register_command(self.alias, self.cmd, desc=self.cmd_desc)
        self.gcode.register_mux_command('SET_GCODE_VARIABLE', 'MACRO', name, self.cmd_SET_GCODE_VARIABLE, desc=self.cmd_SET_GCODE_VARIABLE_help)
    ...
    def handle_connect(self):
        ...
        if prev_cmd is None:
            raise self.printer.config_error('{"code":"key169", "msg": "Existing command \'%s\' not found in gcode_macro rename", "values": ["%s"]}' % (self.alias, self.alias))
